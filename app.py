...
if uploaded_file is not None:
    ...
    # st.write(df.columns.tolist())# ✅ fixed CSV read
    ...
    user_col = find_column(df, possible_user_cols)
    ...
    review_col = find_column(df, possible_review_cols)
    ...
    product_col = find_column(df, possible_product_cols)
    ...
    category_col = find_column(df, possible_category_cols)
    ...
    rating_col = find_column(df, possible_rating_cols)
    ...
    user_list = df[user_col].astype(str).unique().tolist()
    ...
    if st.sidebar.button('show analysis'):
       ...
       if selected_user == 'Overall':
          ...
          if category_col:
              ...
              category_rating = (
                  df.groupby("main_category")[rating_col]
                  .mean()
                  .round(2)
              )
          ...
          col1 ,col2= st.columns(2)
          d = {
              'Category': category_rating.index,
              'Average Rating': category_rating.values
          }
       ...
       count_col = timeline.columns[2]

       ax.bar(
           timeline['review_timeline'].head(50),
           timeline[count_col].head(50)
       )
       ...
       with col2:
           ...
           if not emoji_df.empty:
               ...
               st.info("No emojis found in this dataset.")
           st.pyplot(fig)
...
###########################################################################
       ...
       df['rating_tone'] = df[rating_col].astype(float).apply(rating_tone)
       tone_matrix = pd.crosstab(
           df[product_col],
           df['rating_tone']
       )
    ...
    if uploaded_file is not None:
        ...
        if "model" not in st.session_state:
            st.session_state.model, st.session_state.vectorizer,st.session_state.accuracy = helper.train_sentiment_model(df)
